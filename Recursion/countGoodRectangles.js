/*
    ...
    Explanation: The largest squares you can get from each rectangle are
    of lengths [5,3,5,5].
    The largest possible square is of length 5, and you can get it out of
    3 rectangles.
...
var countGoodRectangles = function(rectangles) {
    ...
    for(let i = 0; i < rectangles.length; i++) {
        if(rectangles[i][0] < rectangles[i][1]) {
            hm[rectangles[i][0]] = hm[rectangles[i][0]] ? hm[rectangles[i][0]] += 1 : 1;
        } else {
            hm[rectangles[i][1]] = hm[rectangles[i][1]] ? hm[rectangles[i][1]] += 1 : 1;;
        }
    }
    
    let squares = Object.keys(hm).sort();

    return hm[squares[squares.length - 1]];
};
